...
def _cur_state() -> dict:
    """Читає curriculum_v2.json і повертає в legacy shape (completed/started/notes).
    Legacy споживачі (get_hub tool) працюють без змін.
    ...
    if v2.exists():
        ...
        topics = d.get("topics", [])
        completed = [t["id"] for t in topics if t.get("state") == "mastered"]
        started   = [t["id"] for t in topics if t.get("state") == "active"]
        return {"completed": completed, "started": started, "notes": {}, "not_started": []}
    legacy = data_dir / "curriculum.json"
    if legacy.exists():
